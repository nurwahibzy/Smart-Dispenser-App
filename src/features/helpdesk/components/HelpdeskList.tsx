...
export default function AdminHelpdeskList() {
    ...
    type TimestampOrDate = string | number | Date | { toDate: () => Date } | null | undefined;
    ...
    const handleSaveStatus = async () => {
        ...
        try {
            ...
            setTickets(
                tickets.map((t) =>
                    t.id === selectedTicket.id ? { ...t, status: draftStatus as HelpdeskTicket['status'] } : t,
                ),
            );
            setSelectedTicket({
                ...selectedTicket,
        ...
        return (
            <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 text-sm border-b pb-6">
                    <div>
                        <p className="text-gray-500 font-medium mb-1">Informasi Pengirim:</p>
                        <p className="font-semibold text-gray-900">{selectedTicket.name}</p>
                        <p className="text-gray-600">{selectedTicket.contact}</p>
                    </div>

                    <div>
                        <p className="text-gray-500 font-medium mb-1">Ubah Status Laporan:</p>
                        <div className="flex items-start gap-2 mt-1">
                            <Select
                                value={draftStatus}
                                onChange={(val) => setDraftStatus(val as HelpdeskTicket['status'])}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div className=" flex flex-col sm:flex-row items-start sm:items-center justify-start gap-4 p-3 border-b">
                <input
                    ...
                    className="px-3 py-2 bg-white text-gray-900 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 focus:outline-none w-full sm:w-64 transition-colors"
                />
                <Select
                    value={filterStatus}
            ...
            {isLoading ? (
                <div className="p-8 text-center text-gray-500">Memuat data laporan...</div>
            ) : error ? (
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {filteredTickets.map((ticket) => (
                                <tr key={ticket.id} className="hover:bg-gray-50 transition-colors">
                                    <td className="p-4 text-gray-600">{formatDate(ticket.createdAt)}</td>
...
const StatusBadge = ({ status }: { status: string }) => {
    ...
    return <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${currentStyle}`}>{currentLabel}</span>;
};
